  ...
  (function (t) {
    var e = {
              ...
              setTimeout(function () {
                i.removeClass("load-more-overlay loading"),
                  Wolmart.Minipopup.open({
                    ...
                    actionTemplate:
                    '<a href="/basket/" class="btn btn-rounded btn-sm">View Cart</a><a href="/checkout/" class="btn btn-dark btn-rounded btn-sm">Checkout</a>',
                  });
              }, 500));
        }
      ),
      Wolmart.$body.on(
        ...
        function (e) {
            ...
            var i = $(this);  // Select the clicked element
            var actionUrl = i.data('url');  // Get the URL to send the POST request to
            ...
            .then(data => {
                // Simulate loading behavior and update the icon
                setTimeout(function () {
                    i.removeClass("load-more-overlay loading");
                    i.toggleClass("w-icon-heart w-icon-heart-full");  // Toggle between empty and full heart
                }, 500);
            })
            .catch(error => {
                console.error('Error:', error);
                alert('An error occurred while processing your request.');
                i.removeClass("load-more-overlay loading");  // Ensure loading is removed in case of error
            });
        }
    );
    ...
    function getCSRFToken() {
        ...
        Wolmart.$body.on("click", ".btn-quickview", function (i) {
          ...
          const pk = $(this).data("product-id");
      
          $.ajax({
            url: `/catalogue/quick_view/${pk}/`,  // Make sure this matches your Django URL route
            method: 'GET',
            success: function(response) {
              // Insert the rendered HTML from the response into the modal content
              $(".product-popup .quickview-content").html(response.html);
      
              // Open the popup modal using Wolmart's popup functionality
              Wolmart.popup(
                {
                  items: { src: e[0].outerHTML },
                  callbacks: {
                    open: function () {
                      // Initialize the product details inside the modal (if necessary)
                      Wolmart.productSingle(t(".mfp-product .product-single"));
                    },
                    close: function () {
                      // Destroy the slider instance when the modal is closed
            ...
            error: function(xhr, status, error) {
              // Handle any AJAX errors (e.g., show a message or log)
    ...
    function n(e) {
      ...
      if (i.hasClass("disabled"))
        ...
        if (a.length)
          ...
          var n =
            '<div class="alert alert-success alert-cart-product mb-2">                            <a href="/basket/" class="btn btn-success btn-rounded">View Cart</a>                            <p class="mb-0 ls-normal">"' +
